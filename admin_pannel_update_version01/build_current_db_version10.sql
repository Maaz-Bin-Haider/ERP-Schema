--
-- Single-file build: CURRENT DATABASE v10 (full schema + all feature changes consolidated)
-- Includes: parties.created_by recorded on insert (add_party_from_json)
-- Target: an EMPTY PostgreSQL 16 database. Run once, then run Django migrations:
--   createdb mydb && psql -v ON_ERROR_STOP=1 -d mydb -f build_current_db.sql
--   python manage.py migrate          # creates the authentication permissions
--
-- Contents (in order): full schema + framework tables; framework-migration seed;
-- Opening Cash feature; core Chart-of-Accounts seed; Monthly Reports (Pakistan:
-- physical-stock Company Position + auto Income Statement); Retained Earnings seed;
-- Owner Equity feature. Objects are owner-neutral (owned by the connecting role).
--
...
CREATE FUNCTION public.detailed_ledger2(p_party_name text, p_start_date date, p_end_date date) RETURNS TABLE(entry_date date, journal_id bigint, description text, party_name text, account_type text, debit numeric, credit numeric, running_balance numeric, invoice_details jsonb, created_by text)
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE FUNCTION public.get_accounts_payable_json_excluding(p_exclude_names text[] DEFAULT ARRAY[]::text[]) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
DECLARE
    result JSONB;
...
CREATE FUNCTION public.get_party_balance_by_name(p_name text) RETURNS json
    LANGUAGE plpgsql STABLE
    AS $$
DECLARE
    v_result JSON;
...
CREATE FUNCTION public.get_party_balances_json_excluding(p_exclude_names text[] DEFAULT ARRAY[]::text[]) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
DECLARE
    result JSONB;
...
CREATE FUNCTION public.item_transaction_history(p_item_name text, p_from_date date DEFAULT NULL::date, p_to_date date DEFAULT NULL::date) RETURNS TABLE(item_name text, serial_number text, serial_comment text, transaction_date date, transaction_type text, counterparty text, price numeric)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE FUNCTION public.sale_wise_profit(p_from_date date, p_to_date date) RETURNS TABLE(sale_date date, item_name text, serial_number text, serial_comment text, sale_price numeric, purchase_price numeric, profit_loss numeric, profit_loss_percent numeric, vendor_name text)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
COMMENT ON FUNCTION public.trg_fn_soldunits_fix_ghost_stock() IS 'Statement-level trigger function. Runs once after each INSERT,
...
CREATE FUNCTION public.update_purchase_invoice(p_invoice_id bigint, p_items jsonb, p_party_name text DEFAULT NULL::text, p_invoice_date date DEFAULT NULL::date, p_created_by integer DEFAULT NULL::integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_item              JSONB;
...
CREATE FUNCTION public.update_sale_return(p_return_id bigint, p_serials jsonb, p_created_by integer DEFAULT NULL::integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    rec           RECORD;
...
COMMENT ON TRIGGER trg_soldunits_fix_ghost_stock ON public.soldunits IS 'Fires once after each full INSERT/UPDATE/DELETE statement on soldunits.
...
-- ─────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.monthly_company_position(p_as_of_date date DEFAULT CURRENT_DATE)
RETURNS json
LANGUAGE plpgsql
STABLE
AS $function$
...
--    Replaces the old 4-argument version (manual sales_revenue + cogs).
-- ─────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.monthly_income_statement(date, date, numeric, numeric);
...
SELECT
    ...
    (SELECT g::text
      ...
      WHERE g::text NOT IN (
                SELECT account_code FROM public.chartofaccounts
    ...
    'Equity'
WHERE NOT EXISTS (
    SELECT 1 FROM public.chartofaccounts WHERE account_name = 'Retained Earnings'
);
...
CREATE TABLE IF NOT EXISTS public.owner_equity_transactions (
    ...
    CONSTRAINT owner_equity_account_fk FOREIGN KEY (equity_account_id)
        REFERENCES public.chartofaccounts (account_id),
...
COMMENT ON TABLE public.owner_equity_transactions IS
...
CREATE OR REPLACE FUNCTION public.add_owner_equity_txn(p_data jsonb)
RETURNS jsonb
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    IF v_direction = 'withdrawal' THEN
        ...
        INSERT INTO public.journallines (journal_id, account_id, debit)  VALUES (v_journal, v_equity_acc, v_amount);
        INSERT INTO public.journallines (journal_id, account_id, credit) VALUES (v_journal, v_cash_acc,   v_amount);
    ELSE
        -- Debit Cash (cash in) / Credit equity (increase capital)
        INSERT INTO public.journallines (journal_id, account_id, debit)  VALUES (v_journal, v_cash_acc,   v_amount);
        INSERT INTO public.journallines (journal_id, account_id, credit) VALUES (v_journal, v_equity_acc, v_amount);
    END IF;

    INSERT INTO public.owner_equity_transactions
...
CREATE OR REPLACE FUNCTION public.get_owner_equity_json(p_limit integer DEFAULT 50)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
AS $function$
...
BEGIN
    ...
    FROM (
        ...
        FROM   public.owner_equity_transactions t
        JOIN   public.chartofaccounts c ON c.account_id = t.equity_account_id
    ...
    INTO   v_inj, v_wd
    FROM   public.owner_equity_transactions;
...
CREATE OR REPLACE FUNCTION public.delete_owner_equity_txn(p_txn_id bigint)
RETURNS jsonb
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    FROM   public.owner_equity_transactions WHERE txn_id = p_txn_id;
    ...
    UPDATE public.owner_equity_transactions SET journal_id = NULL WHERE txn_id = p_txn_id;
    ...
    END IF;
    DELETE FROM public.owner_equity_transactions WHERE txn_id = p_txn_id;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.detailed_ledger(p_party_name text, p_start_date date, p_end_date date)
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.detailed_ledger2(p_party_name text, p_start_date date, p_end_date date)
