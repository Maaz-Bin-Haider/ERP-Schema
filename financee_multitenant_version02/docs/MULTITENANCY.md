...
| `tenancy/admin_activity_patch.py` | Adapter that makes the admin "User Activity" page aggregate across **all** tenant schemas (see §13). |
...
| Cross‑tenant activity adapter: counts/details merge across schemas, search_path restored | ✅ |
| Adapter installs via `ready()`, system check clean, idempotent re‑apply | ✅ |
...
2. **Superusers** default to `public` (so the admin always works). A superuser who also needs business‑UI data can be given a Membership; otherwise the admin's cross‑schema "User Activity" page now aggregates across **all** tenant schemas via the included adapter (§13), so it shows real cross‑tenant totals rather than degrading to empty. The adapter can be turned off with `TENANCY_CROSS_TENANT_ACTIVITY = False`.
...
## 13. Cross‑tenant "User Activity" admin page

The custom admin's **User Activity** page (in `financee/admin_site.py`) runs raw
SQL against the business tables through the connection's `search_path`. Because
an admin request is served under `public` (a superuser has no Membership), those
tables aren't on the path and the page's defensive `try/except` blocks would
otherwise return empty.

`tenancy/admin_activity_patch.py` restores cross‑tenant aggregation **without
editing `admin_site.py`**. At startup (`TenancyConfig.ready()`) it monkey‑patches
three module‑level functions so the existing views transparently loop over every
provisioned tenant schema and merge the results:

* `_collect_activity` → per‑user counts summed across all tenant schemas.
* `build_detailed_activity` → one user's detailed feed merged across schemas and
  re‑sorted newest‑first.
* `build_user_activity` → reuses the now cross‑tenant counts and relabels each
  row's "schema" column with that user's **actual** company schema (from the
  registry).
* `_app_schema` → display label `"all tenants"`.

The originals are preserved and called once per schema, so no SQL is duplicated;
patching module globals works because the views resolve these names from the
`admin_site` namespace at call time. Each per‑schema pass saves and restores the
prior `search_path`, so it never leaks tenant context. The adapter is idempotent
and controlled by a settings flag:

```python
# settings.py — optional; defaults to True when unset
TENANCY_CROSS_TENANT_ACTIVITY = True
```

**Cost:** the page runs its activity queries once per tenant schema, so work
scales with the number of tenants. It's an admin‑only screen, so this is
normally fine; set the flag to `False` to revert to the per‑request‑schema
(`public`) behaviour.
