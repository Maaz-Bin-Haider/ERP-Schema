--
-- Single-file build: FULL SCHEMA + framework migration seed (run this, THEN: manage.py migrate)
-- Target: an EMPTY PostgreSQL 16 database. Run once, e.g.:
--   createdb mydb && psql -v ON_ERROR_STOP=1 -d mydb -f build_schema_only.sql
-- Source of truth: production dump db_backup_20260610_0000.sql (reconciled, verified equivalent).
--
...
ALTER TABLE public.opening_cash OWNER TO myuser;
...
ALTER FUNCTION public.get_opening_cash_json() OWNER TO myuser;
...
ALTER FUNCTION public.set_opening_cash_from_json(jsonb) OWNER TO myuser;
...
BEGIN
    ...
    FROM (
        ...
        WHERE  c.account_type = 'Expense'
          AND  c.account_name NOT ILIKE '%cost of goods%'   -- inventory cost is captured by Purchases
          AND  c.account_name NOT ILIKE '%profit%'          -- PROFIT / LABELS PROFIT are not operating costs
          AND  je.entry_date BETWEEN p_from_date AND p_to_date
        GROUP  BY c.account_name
    ) ex;
