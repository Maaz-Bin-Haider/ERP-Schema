-- ============================================================================
--  migrate_existing_to_tenant.sql
--  ---------------------------------------------------------------------------
--  PRODUCTION conversion: move an EXISTING, POPULATED single-schema database
--  (all business data currently in `public`) into the first tenant schema
--  `tenant_company_1`, WITHOUT losing data.
--
--  Run order (see DOCS):
--     0. pg_dump backup first!
--     1. deploy new code, then:  python manage.py migrate
--        (creates tenancy_company / tenancy_membership + new permissions)
--     2. psql -v ON_ERROR_STOP=1 -d <db> -f migrate_existing_to_tenant.sql
--
--  Strategy:
--     A. Drop business VIEWS / FUNCTIONS / TRIGGERS from public (logic only,
--        no data) using a catalog-driven sweep (public holds no Django funcs).
--     B. Move the 22 data-bearing business TABLES + 21 SEQUENCES into the
--        tenant schema with ALTER ... SET SCHEMA (DATA + constraints preserved;
--        the cross-schema created_by -> public.auth_user FK stays valid).
--     C. Recreate all business logic schema-relative inside the tenant schema.
--     D. Register the tenant + map every existing user to it (one user = one
--        company), so logins immediately resolve to tenant_company_1.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- A. Drop business logic from public (views, then routines+their triggers).
--    public contains no Django/auth functions or views, so this is safe.
-- ----------------------------------------------------------------------------
DO $sweep$
DECLARE r record;
BEGIN
...
-- ----------------------------------------------------------------------------
-- B. Create the tenant schema and move data-bearing objects into it.
-- ----------------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS tenant_company_1;

-- move tables (data preserved; owned sequences follow automatically)
ALTER TABLE IF EXISTS public.chartofaccounts SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.contra_entries SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.items SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.journalentries SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.journallines SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.opening_cash SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.owner_equity_transactions SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.parties SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.payments SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.period_closes SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.purchaseinvoices SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.purchaseitems SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.purchasereturnitems SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.purchasereturns SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.purchaseunits SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.receipts SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.salesinvoices SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.salesitems SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.salesreturnitems SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.salesreturns SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.soldunits SET SCHEMA tenant_company_1;
ALTER TABLE IF EXISTS public.stockmovements SET SCHEMA tenant_company_1;

-- move any remaining standalone sequences (ref-number generators)
ALTER SEQUENCE IF EXISTS public.chartofaccounts_account_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.contra_ref_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.items_item_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.journalentries_journal_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.journallines_line_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.parties_party_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.payments_payment_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.payments_ref_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.purchaseinvoices_purchase_invoice_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.purchaseitems_purchase_item_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.purchasereturnitems_return_item_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.purchasereturns_purchase_return_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.purchaseunits_unit_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.receipts_receipt_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.receipts_ref_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.salesinvoices_sales_invoice_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.salesitems_sales_item_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.salesreturnitems_return_item_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.salesreturns_sales_return_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.soldunits_sold_unit_id_seq SET SCHEMA tenant_company_1;
ALTER SEQUENCE IF EXISTS public.stockmovements_movement_id_seq SET SCHEMA tenant_company_1;

-- ----------------------------------------------------------------------------
-- C. Recreate all business logic INSIDE the tenant schema (schema-relative).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- D. Register the tenant and map every existing user to it.
--    (Single-tenant source => all current users belong to Company One.)
-- ----------------------------------------------------------------------------
INSERT INTO public.tenancy_company (name, schema_name, is_active)
VALUES ('Company One', 'tenant_company_1', true)
ON CONFLICT (schema_name) DO NOTHING;

INSERT INTO public.tenancy_membership (user_id, company_id)
SELECT u.id, c.id
FROM public.auth_user u
CROSS JOIN public.tenancy_company c
WHERE c.schema_name = 'tenant_company_1'
ON CONFLICT (user_id) DO NOTHING;

COMMIT;

-- Verify:  SELECT relname, relnamespace::regnamespace
--          FROM pg_class WHERE relname IN ('items','salesinvoices');
